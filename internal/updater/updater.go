...
func New(cfg *config.UpdateConfig, db database.Database, logger *slog.Logger) *Updater {
	...
	return &Updater{cfg: cfg, db: db, client: &http.Client{Timeout: timeout}, logger: logger}
}

// ============================================================================
// Main orchestrator — concurrent feed downloads
// ============================================================================
...
func (u *Updater) RunAll(ctx context.Context) (*UpdateStats, error) {
	...
	// Check KEV version without downloading full body — just HEAD.
	if kevVersion != "" {
		kevCurrent, kevChanged = u.checkKEVVersion(ctx)
		if !kevChanged {
			fmt.Println("KEV: Already up to date")
		} else {
			fmt.Println("KEV: Updates available")
		}
	} else {
		fmt.Println("KEV: First download")
	}

	// Check EPSS date without downloading.
	if epssDate != "" {
		epssChanged = u.checkEPSSDate(ctx, epssDate)
		if !epssChanged {
			fmt.Println("EPSS: Already up to date")
		} else {
			fmt.Println("EPSS: Updates available")
		}
	} else {
		fmt.Println("EPSS: First download")
	}
	fmt.Println()
	...
	go func() {
		defer wg.Done()
		ins, upd, err := u.updateKEV(ctx, kevVersion, kevCurrent, kevChanged)
	...
	go func() {
		defer wg.Done()
		ins, upd, err := u.updateEPSS(ctx, epssDate, epssChanged)
...
func (u *Updater) fetchWithRetry(ctx context.Context, url string) ([]byte, error) {
	...
	for attempt := 0; attempt <= u.cfg.RetryCount; attempt++ {
		if attempt > 0 {
			delay, _ := time.ParseDuration(u.cfg.RetryDelay)
			select {
	...
	return nil, fmt.Errorf("all %d attempts failed: %w", u.cfg.RetryCount+1, lastErr)
}

// ============================================================================
// KEV version check — parses catalogVersion from JSON without full processing
// ============================================================================
...
// ============================================================================
// EPSS date check — parses score_date from CSV comment without full processing
// ============================================================================
...
func (u *Updater) checkEPSSDate(ctx context.Context, storedDate string) bool {
	...
	for {
		...
		if len(row) >= 1 && strings.HasPrefix(row[0], "#") {
			// Parse score_date from comment line: #model_version:...,score_date:YYYY-MM-DD
...
// ============================================================================
// CVE Update — NVD with incremental + transaction
// ============================================================================
...
func (u *Updater) updateCVE(ctx context.Context, lastUpdate string) (int, int, error) {
	...
	if isFull {
		u.logger.Info("full CVE download from NVD")
	} else {
		u.logger.Info("incremental CVE update", "since", lastUpdate)
	}
	...
	for si := 0; ; si += 100 {
		if ctx.Err() != nil {
			u.recordHistory(start, "NVD", "interrupted", inserted, updated, ctx.Err())
		...
		if err != nil {
			u.recordHistory(start, "NVD", "failed", inserted, updated, err)
		...
		if progress < 50 { bar += ">" }
		fmt.Printf("\rCVE Download: [%-50s] %.0f%% (%d of %d pages)", bar, pct, page+1, (resp.TotalResults+99)/100)
		page++
	...
	u.recordHistory(start, "NVD", "success", inserted, updated, nil)
	return inserted, updated, nil
}

// ============================================================================
// KEV Update — with version tracking
// ============================================================================
...
func (u *Updater) updateKEV(ctx context.Context, storedVersion, currentVersion string, changed bool) (int, int, error) {
	if !changed {
		return 0, 0, nil
	}
	...
	body, err := u.fetchWithRetry(ctx, u.cfg.KEVBaseURL)
	if err != nil {
		u.recordHistory(start, "KEV", "failed", 0, 0, err)
		return 0, 0, fmt.Errorf("fetching: %w", err)
	}
	...
	// Store the version regardless of whether data changed.
	...
	if len(entries) == 0 {
		u.recordHistory(start, "KEV", "success", 0, 0, nil)
		return 0, 0, nil
	}
	...
	u.recordHistory(start, "KEV", "success", len(entries), 0, nil)
	return len(entries), 0, nil
}

// ============================================================================
// EPSS Update — with date tracking
// ============================================================================

func (u *Updater) updateEPSS(ctx context.Context, storedDate string, changed bool) (int, int, error) {
	if !changed {
		return 0, 0, nil
	}
	...
	body, err := u.fetchWithRetry(ctx, u.cfg.EPSSBaseURL)
	if err != nil {
		u.recordHistory(start, "EPSS", "failed", 0, 0, err)
		return 0, 0, fmt.Errorf("fetching: %w", err)
	}
	...
	// Need a new reader since we consumed the gzip.
	gz.Close()
	gz2, _ := gzip.NewReader(strings.NewReader(string(body)))
	defer gz2.Close()
	...
	if len(entries) == 0 {
		...
		u.recordHistory(start, "EPSS", "success", 0, 0, nil)
		return 0, 0, nil
	}
	...
	for i := 0; i < len(entries); i += 1000 {
		...
		if err := u.db.EPSS().BulkUpsert(ctx, entries[i:end]); err != nil {
			u.recordHistory(start, "EPSS", "failed", total, 0, err)
	...
	u.recordHistory(start, "EPSS", "success", total, 0, nil)
	return total, 0, nil
}
...
func (u *Updater) recordHistory(start time.Time, feed, status string, inserted, updated int, err error) {
	...
	// Write to history table (best-effort).
	ctx := context.Background()
	u.db.Metadata().Set(ctx, fmt.Sprintf("history_%s_%d", feed, start.Unix()),
